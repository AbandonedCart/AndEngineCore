...
public class ParallaxLayer extends Entity {
	...
	public ParallaxLayer(final Camera camera, final boolean mIsScrollable,
			final int mLevelWidth, final boolean mParallaxOnAxisX) {
		this.mCamera = camera;
		this.mIsScrollable = mIsScrollable;
		this.mLevelScale = mLevelWidth;
		ParallaxLayer.mParallaxOnAxisX = mParallaxOnAxisX;

		if (mParallaxOnAxisX)
			mCameraPreviousPos = camera.getCenterX();
		else
			mCameraPreviousPos = camera.getCenterY();
	}
	...
	public void onManagedDraw(GLState pGLState, Camera pCamera) {
		...
		for (int i = 0; i < this.mParallaxEntityCount; i++) {
			if (parallaxEntities.get(i).mIsScrollable) {
				parallaxEntities.get(i).onDraw(pGLState, pCamera,
						parallaxScrollValue, mLevelScale, mParallaxOnAxisX);
			} else {
				parallaxEntities.get(i).onDraw(pGLState, pCamera,
						parallaxValue, mLevelScale, mParallaxOnAxisX);
			}
	...
	protected void onManagedUpdate(float pSecondsElapsed) {
		...
		final float cameraCenterPos = (mParallaxOnAxisX) ? this.mCamera
				.getCenterX() : this.mCamera.getCenterY();
		...
		if (mIsScrollable && mCameraPreviousPos != cameraCenterPos) {
			...
			this.mParallaxScrollValue += mCameraOffsetPos
					* this.mParallaxScrollFactor;
			mCameraOffsetPos = 0;
		}
	...
	public static class ParallaxEntity {
		...
		public ParallaxEntity(final float pParallaxFactor,
				final IAreaShape pAreaShape) {
			this.mParallaxFactor = pParallaxFactor;
			this.mAreaShape = pAreaShape;
			this.mIsScrollable = false;

			shapeScaled = (mParallaxOnAxisX) ? this.mAreaShape.getWidthScaled()
					: this.mAreaShape.getHeightScaled();
		}

		public ParallaxEntity(final float pParallaxFactor,
				final IAreaShape pAreaShape, final boolean mIsScrollable) {
			this.mParallaxFactor = pParallaxFactor;
			this.mAreaShape = pAreaShape;
			this.mIsScrollable = mIsScrollable;
		...
		public ParallaxEntity(final float pParallaxFactor,
			...
			shapeScaled = (mParallaxOnAxisX) ? this.mAreaShape.getWidthScaled()
					* mReduceFrequency : this.mAreaShape.getHeightScaled()
					* mReduceFrequency;
		}
		...
		public void onDraw(final GLState pGLState, final Camera pCamera,
				final float pParallaxValue, final float mLevelScale,
				final boolean mParallaxOnAxisX) {
			pGLState.pushModelViewGLMatrix();
			{
				...
				if (mLevelScale != 0) {
					...
					range = (mParallaxOnAxisX) ? pCamera.getWidth() : pCamera
							.getHeight();
				}
				...
				do {
					...
					if (mParallaxOnAxisX)
						pGLState.translateModelViewGLMatrixf(shapeScaled - 1,
								0, 0);
					else
